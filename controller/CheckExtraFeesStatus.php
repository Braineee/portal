...
$get_balance_info = DB_STUDENT::getInstance()
->query("SELECT * FROM Extrafee WHERE Matricno like '{$_SESSION['student_details']->matricnum}'");
...
if ($get_balance_info->count() > 0) {
    ...
    $query_ = "SELECT * FROM vw_YCTPAY_Transactions
                                ...
                                AND SessionID LIKE '$new_session' 
                                AND PayeeNum LIKE '$user_matricnum' 
                                AND Amount LIKE '$balance_amount%'
                                AND TransactionStatus like '%Successful%'";
    $check_balance_payment = DBMSQL::getInstance()->query($query_);
